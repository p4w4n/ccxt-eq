...
var zerodha$1 = require('./abstract/zerodha.js');
 ...
 */
class zerodha extends zerodha$1 {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    'TokenException': errors.AuthenticationError,
                    'UserException': errors.PermissionDenied,
                    'OrderException': errors.InvalidOrder,
                    'InputException': errors.BadRequest,
                    'MarginException': errors.InsufficientFunds,
                    'HoldingException': errors.InsufficientFunds,
                    'NetworkException': errors.NetworkError,
                    'DataException': errors.ExchangeError,
                    'GeneralException': errors.ExchangeError,
                },
                'broad': {
                    'Invalid API credentials': errors.AuthenticationError,
                    'Insufficient funds': errors.InsufficientFunds,
                    'Order not found': errors.InvalidOrder,
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (product === undefined) {
            throw new errors.InvalidOrder(this.id + ' createOrder() requires the "product" parameter (e.g., "CNC", "MIS") in params');
        }
        ...
        if (type === 'limit') {
            if (price === undefined) {
                throw new errors.InvalidOrder(this.id + ' createOrder() requires a price argument for limit orders');
            }
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        ...
        if (!Array.isArray(orders) || orders.length === 0) {
            throw new errors.InvalidOrder(this.id + ' order ' + id + ' not found');
        }
    ...
    sign(path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            if (!this.password) {
                throw new errors.AuthenticationError(this.id + ' access token is missing. Please provide it in the "password" field or use the generate_token.py script.');
            }
            headers = {
    ...
    handleErrors(code, reason, url, method, headers, body, response, requestHeaders, requestBody) {
        ...
        if (code === 429) {
            throw new errors.RateLimitExceeded(this.id + ' rate limit exceeded');
        }
        // Handle server errors
        if (code >= 500) {
            throw new errors.ExchangeNotAvailable(this.id + ' server error: ' + body);
        }
    }
}
